...
class WhatsAppFollowUp(models.Model):
    ...
    def message_context(self, settings: WhatsAppSettings | None = None) -> dict:
        ...
        if self.last_service_date:
            delta_days = (timezone.localdate() - self.last_service_date).days
            days_since_service = str(max(delta_days, 0))
            last_service_str = self.last_service_date.strftime("%B %d, %Y")
        else:
            days_since_service = "0"
            last_service_str = "N/A"
        context = {
    ...
    def build_message(self, settings: WhatsAppSettings | None = None) -> str:
        ...
        template = WhatsAppSettings.MESSAGE_TEMPLATE
        return template.format_map(_DefaultDict(self.message_context(settings=settings)))

    def content_variables(self, settings: WhatsAppSettings | None = None) -> dict[str, str]:
        settings = settings or WhatsAppSettings.load()
        context = self.message_context(settings=settings)
        return {
            "1": context.get("client_name", ""),
            "2": context.get("business_name", ""),
            "3": context.get("days_since_service", "0"),
