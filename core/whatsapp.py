...
def _content_sid() -> str:
    ...
    if not content_sid:
        raise WhatsAppConfigurationError(
            "Twilio WhatsApp content template is not configured. Set TWILIO_CONTENT_SID in the environment."
        )
    return content_sid.strip()
...
def send_follow_up_message(
    ...
    *,
    ...
    content_variables = json.dumps(follow_up.content_variables(settings=settings_obj))
